...
function normalizeIndustryQuery(industry = '') {
  const normalized = industry.trim().toLowerCase()

  if (normalized === 'estudios contables' || normalized === 'estudio contable') {
    return 'estudio contable accounting firm tax advisory'
  }
  if (normalized === 'contadores' || normalized === 'contador') {
    return 'contador estudio contable accounting firm tax advisory'
  }
  if (normalized === 'abogados' || normalized === 'estudio juridico' || normalized === 'estudio jurídico') {
    return 'estudio juridico abogados law firm legal services'
  }
...
function buildSearchQuery(country, city = '', industry = '') {
  ...
  const industryLabel = normalizeIndustryQuery(industry).trim() || 'business'
...
export async function searchGoogleCustomSearch(country, city = '', industry = '') {
...
export async function searchBing(country, city = '', industry = '') {
  const { locale } = getCountryMeta(country)
  const query = buildSearchQuery(country, city, industry)
  ...
  return [...dom.window.document.querySelectorAll('li.b_algo')]
    .map((node) => {
      const title = normalizeWhitespace(node.querySelector('h2')?.textContent || '')
      const rawUrl = node.querySelector('h2 a')?.href || ''
...
export async function searchWeb(country, city = '', industry = '') {
  const [googleResults, duckDuckGoResults, bingResults] = await Promise.allSettled([
    searchGoogleCustomSearch(country, city, industry),
    searchDuckDuckGo(country, city, industry),
    searchBing(country, city, industry)
  ])

  const merged = []
  if (googleResults.status === 'fulfilled') merged.push(...googleResults.value)
  if (duckDuckGoResults.status === 'fulfilled') merged.push(...duckDuckGoResults.value)
  if (bingResults.status === 'fulfilled') merged.push(...bingResults.value)

  if (!merged.length && country && country !== 'Global') {
    const seedResults = await searchSeed(country, city, industry)
    merged.push(...seedResults)
  }
...
export function isRelevantToIndustry(prospect, industry = '') {
  ...
  const loweredHaystack = `${host} ${haystack.toLowerCase()}`
  if (/(\.gob\.|\.gov$|museum|wikipedia|wikidata)/.test(loweredHaystack)) return false
  if (industryTokens.includes('contables') || industryTokens.includes('contable')) {
    return /(estudio contable|contable|impositivo|tributario|contador|contadores|accounting|tax|bookkeeping|audit|auditoria)/.test(loweredHaystack)
  }
...
async function searchProspectsOnce({ countries = DEFAULT_COUNTRIES, city = '', industry = '', limit = 12 } = {}) {
  const safeCountries = countries.filter((country) => ALL_COUNTRIES.includes(country))
  const useGlobal = !safeCountries.length || safeCountries.includes('Global')
  const selectedCountries = useGlobal ? ['Global'] : safeCountries
    ...
    searchResults
      .map((item) => {
        ...
        return {
          ...
          sourceUrl: item.url,
          snippet: item.snippet,
          country: item.detectedCountry || (useGlobal ? 'Global' : selectedCountries[0]),
          city: item.city || city || '',
...
export async function searchProspects({ countries = DEFAULT_COUNTRIES, city = '', industry = '', limit = 12 } = {}) {
  const attempts = [
    { countries, city, industry, limit }
  ]

  if (city.trim()) {
    attempts.push({ countries, city: '', industry, limit })
  }

  const normalizedCountries = countries.filter((country) => ALL_COUNTRIES.includes(country))
  const isGlobal = !normalizedCountries.length || normalizedCountries.includes('Global')
  if (!isGlobal) {
    attempts.push({ countries: ['Global'], city: city.trim(), industry, limit })
    if (city.trim()) {
      attempts.push({ countries: ['Global'], city: '', industry, limit })
    }
  }
  ...
  for (const attempt of attempts) {
    ...
    seen.add(key)
    const result = await searchProspectsOnce(attempt)
