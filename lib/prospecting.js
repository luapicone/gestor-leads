...
const stopHosts = ['facebook.com', 'instagram.com', 'linkedin.com', 'youtube.com', 'x.com', 'twitter.com', 'wikipedia.org', 'wikidata.org']
...
export async function searchSeed(country, city = '') {
...
export async function searchWeb(country, city = '') {
  return searchSeed(country, city)
}
...
export function isConstructionRelevant(prospect) {
  const haystack = [
    prospect.name,
    prospect.snippet,
  ...
  if (/(\.gob\.|\.gov$|consejo|premio|award|museum|banco)/.test(`${host} ${haystack}`)) return false

  return /(constructora|construction|constructor|edific|arquitect|ingenier|rehabilit|inmobiliaria|promotora|residencial|vivienda|contractor|builder)/.test(haystack)
}
...
export async function searchProspects({ countries = DEFAULT_COUNTRIES, city = '', limit = 12 } = {}) {
  const searches = countries.map(async (country) => {
    const results = await searchWeb(country, city)
  ...
  const prospects = dedupeByUrl(
    searchResults
      .map((item) => {
        ...
        const country = item.detectedCountry || countries.find((c) => item.title.includes(c) || item.snippet.includes(c)) || countries[0]
        return {
  ...
  for (const prospect of prospects) {
    ...
    if (isConstructionRelevant(enrichedProspect)) {
